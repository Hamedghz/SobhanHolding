...
final class RuleRepository
{
    ...
    public static function publish(int $decisionId,array $data,int $actorId):int{
        $decision=ManagementMeetingsRepository::getDecisionById($decisionId,Auth::user());if(!$decision||(int)$decision['is_rule']!==1)throw new DomainException('این مصوبه به عنوان قانون علامت‌گذاری نشده است.');$meeting=ManagementMeetingsRepository::getMeetingById((int)$decision['meeting_id'],Auth::user());if(($decision['followup_status']??'')!=='verified'&&($meeting['status']??'')!=='finalized')throw new DomainException('پیش از انتشار قانون، جلسه را نهایی یا مصوبه را تأیید کنید.');return ManagementMeetingsRepository::convertDecisionToRule($decisionId,$data,$actorId);
    }
