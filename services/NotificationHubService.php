...
{
    public const MODULES=['cartable','ticketing','approval','hr','sales','management','system'];
    ...
    public static function clientConfig(array $device): array
    {
        ...
        foreach(self::MODULES as $module){$row=$byModule[$module]??[];$sensitive=in_array($module,['hr','management'],true);$modules[$module]=[
            ...
            'allow_quick_reply'=>false,'direct_action_enabled'=>(bool)($row['direct_action_enabled']??0),
        ];}
    ...
    private static function allowedActions(array $row,array $config): array{$actions=json_decode((string)($row['actions_json']??''),true);if(!is_array($actions))$actions=[['id'=>'open','label'=>'باز کردن'],['id'=>'mark_read','label'=>'خوانده شد']];$out=[];foreach($actions as $a){$id=(string)($a['id']??'');if(in_array($id,['approve','reject'],true)&&empty($config['direct_action_enabled']))continue;if(in_array($id,['open','mark_read','approve','reject','mute','comment','view_cartable','view_ticket'],true))$out[]=['id'=>$id,'label'=>self::text((string)($a['label']??$id),60)];}return array_slice($out,0,5);}
    public static function module(string $event): string{$event=strtolower($event);return match(true){str_contains($event,'ticket'),str_contains($event,'sla')=>'ticketing',str_contains($event,'cartable')=>'cartable',str_contains($event,'approval')=>'approval',str_contains($event,'forwarded_report'),str_contains($event,'sale'),str_contains($event,'manager_dashboard')=>'sales',str_contains($event,'hr'),str_contains($event,'assessment'),str_contains($event,'payroll')=>'hr',str_contains($event,'management'),str_contains($event,'meeting'),str_contains($event,'resolution'),str_contains($event,'finance')=>'management',default=>'system'};}
