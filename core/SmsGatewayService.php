...
class SmsGatewayService
{
    ...
    public function sendSimpleSms(array $mobiles,string $message,?string $sender=null,?int $actorId=null,string $sourceModule='manual',?int $sourceId=null): array
    {
        if(empty($this->config['is_active']))return ['success'=>false,'message'=>'سامانه پیامکی غیرفعال است.','valid_count'=>0,'invalid_count'=>count($mobiles),'batches'=>[]];$original=array_values($mobiles);$valid=$this->normalizeMobiles($original);$invalid=max(0,count($original)-count($valid));$message=trim($message);$sender=trim((string)($sender?:$this->config['default_sender']));if($message===''||!$valid||$sender==='')return ['success'=>false,'message'=>$message===''?'متن پیامک خالی است.':(!$valid?'شماره موبایل معتبری وجود ندارد.':'خط ارسال ثبت نشده است.'),'valid_count'=>count($valid),'invalid_count'=>$invalid,'batches'=>[]];
        $batches=[];foreach(array_chunk($valid,90) as $chunk){$result=$this->sendChunk($chunk,$message,$sender);$id=$this->storeBatch($chunk,$message,$sender,$result,$actorId,$sourceModule,$sourceId,$invalid);$result['message_id']=$id;$batches[]=$result;$invalid=0;}return ['success'=>!array_filter($batches,static fn($b)=>empty($b['success'])),'message'=>'پردازش ارسال پیامک انجام شد.','valid_count'=>count($valid),'invalid_count'=>max(0,count($original)-count($valid)),'batches'=>$batches];
    }
    ...
    private function storeBatch(array $mobiles,string $message,string $sender,array $result,?int $actorId,string $sourceModule,?int $sourceId,int $invalid): int
    {
        $status=$result['success']?'sent':'failed';$pdo=Database::connection();$pdo->beginTransaction();try{Database::execute('INSERT INTO sms_messages(provider_name,sender,message_body,message_hash,recipients_count,valid_recipients_count,invalid_recipients_count,bulk_code,status,source_module,source_id,created_by,sent_at,error_code,error_message,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,IF(?="sent",NOW(),NULL),?,?,NOW())',[(string)($this->config['provider_name']??'bazyabpayam'),$sender,$message,hash('sha256',$message),count($mobiles)+$invalid,count($mobiles),$invalid,$result['bulk_code'],$status,mb_substr($sourceModule,0,100),$sourceId,$actorId,$status,$result['error_code'],$result['error_message']]);$id=(int)Database::lastInsertId();foreach($mobiles as $mobile)Database::execute('INSERT INTO sms_message_recipients(message_id,mobile,normalized_mobile,delivery_status,created_at) VALUES(?,?,?,?,NOW())',[$id,$this->maskSensitive($mobile),$mobile,$status]);$pdo->commit();return $id;}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    }
