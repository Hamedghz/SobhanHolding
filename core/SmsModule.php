...
class SmsModule
{
    public static function repair(PDO $pdo): void
    {
        ...
        $sql=[
            ...
            "CREATE TABLE IF NOT EXISTS sms_messages (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,provider_name VARCHAR(100) NOT NULL DEFAULT 'bazyabpayam',sender VARCHAR(50) NOT NULL,message_body TEXT NOT NULL,message_hash VARCHAR(64) NULL,request_key CHAR(64) NULL,segment_count INT UNSIGNED NOT NULL DEFAULT 1,recipients_count INT UNSIGNED NOT NULL DEFAULT 0,valid_recipients_count INT UNSIGNED NOT NULL DEFAULT 0,invalid_recipients_count INT UNSIGNED NOT NULL DEFAULT 0,bulk_code VARCHAR(100) NULL,status VARCHAR(50) NOT NULL DEFAULT 'queued',source_module VARCHAR(100) NULL,source_id BIGINT UNSIGNED NULL,created_by INT UNSIGNED NULL,sent_at DATETIME NULL,last_checked_at DATETIME NULL,error_code VARCHAR(50) NULL,error_message TEXT NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE KEY uq_sms_request_key(request_key),INDEX idx_sms_messages_status(status),INDEX idx_sms_messages_bulk(bulk_code),INDEX idx_sms_messages_source(source_module,source_id),CONSTRAINT fk_sms_messages_creator FOREIGN KEY(created_by) REFERENCES users(id) ON DELETE SET NULL){$e}",
        ...
        $columns=[
            ...
            'sms_messages'=>['message_hash'=>'VARCHAR(64) NULL','request_key'=>'CHAR(64) NULL','segment_count'=>'INT UNSIGNED NOT NULL DEFAULT 1','valid_recipients_count'=>'INT UNSIGNED NOT NULL DEFAULT 0','invalid_recipients_count'=>'INT UNSIGNED NOT NULL DEFAULT 0'],
        ...
        $requestIndex=(int)$pdo->query("SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='sms_messages' AND INDEX_NAME='uq_sms_request_key'")->fetchColumn();if(!$requestIndex)$pdo->exec('ALTER TABLE sms_messages ADD UNIQUE KEY uq_sms_request_key(request_key)');
