...
class FileBackupModule
{
    public static function repair(PDO $pdo): void
    {
        ...
        $settings=[['file_backup_api_key_hash','','password'],['file_backup_allowed_ips','','text'],['file_backup_enabled','0','boolean'],['file_backup_batch_max','100','number'],['file_backup_max_attempts','5','number']];$stmt=$pdo->prepare('INSERT IGNORE INTO site_settings(setting_key,setting_value,setting_type,updated_at) VALUES(?,?,?,NOW())');foreach($settings as $setting)$stmt->execute($setting);
    }
}
