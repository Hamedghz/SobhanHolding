...
class WorkPlannerModule
{
    public static function repair(PDO $pdo): void
    {
        ...
        $index->execute(['work_planner_tasks','uq_work_planner_client_request']);
        if(!(int)$index->fetchColumn())$pdo->exec('ALTER TABLE work_planner_tasks ADD UNIQUE INDEX uq_work_planner_client_request(user_id,client_request_key)');
