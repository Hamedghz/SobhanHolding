...
class WorkPlannerModule
{
    public static function repair(PDO $pdo): void
    {
        ...
        self::index($pdo,'work_planner_tasks','idx_work_planner_reminder','ALTER TABLE work_planner_tasks ADD INDEX idx_work_planner_reminder(status,due_date,reminder_sent_at)');
    ...
    private static function index(PDO $pdo,string $table,string $index,string $sql): void
    {
        $query=$pdo->prepare('SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND INDEX_NAME=?');$query->execute([$table,$index]);if(!(int)$query->fetchColumn())$pdo->exec($sql);
    }
