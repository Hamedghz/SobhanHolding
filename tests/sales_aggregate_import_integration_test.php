...
$sourceKeyU1001=sha1('sales_aggregate|U-1001');$saved=Database::fetch('SELECT raw_json,quantity,invoice_date FROM sales_aggregate_rows WHERE source_unique_key=?',[$sourceKeyU1001]);$raw=json_decode($saved['raw_json']??'',true);if(($raw['ستون آینده']??'')!=='future-a'||(float)$saved['quantity']!==1200.0||$saved['invoice_date']!=='2024-05-21')throw new RuntimeException('Normalization/raw preservation failed.');
