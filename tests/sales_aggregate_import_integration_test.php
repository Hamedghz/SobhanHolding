...
$headers=['نوع فاکتور','شماره فاکتور','تاریخ','کد فروشنده','نام فروشنده','کد مشتری','نام مشتری','کد کالا','نام کالا','تعداد کل','مبلغ ناخالص','مجموع مبلغ تخفیف سطری','مبلغ خالص','کد یکتا','ستون آینده'];
$rows=[
...
$saved=Database::fetch('SELECT raw_json,quantity,invoice_date FROM sales_aggregate_rows WHERE source_unique_key=?',['unique:U-1001']);$raw=json_decode($saved['raw_json']??'',true);if(($raw['ستون آینده']??'')!=='future-a'||(float)$saved['quantity']!==1200.0||$saved['invoice_date']!=='2024-05-21')throw new RuntimeException('Normalization/raw preservation failed.');
...
$newRows=[$rows[0]];$newRows[0][1]='2001';$newRows[0][13]='U-2001';$path=fixture($headers,$newRows);$cancel=upload($path,'skip_duplicates');@unlink($path);SalesAggregateImportService::rollbackBatch($cancel['batch_id'],1,true);$batch=Database::fetch('SELECT status FROM sales_import_batches WHERE id=?',[$cancel['batch_id']]);if(($batch['status']??'')!=='cancelled'||SalesAggregateRepository::sourceKeyExists('unique:U-2001'))throw new RuntimeException('Batch rollback failed.');
