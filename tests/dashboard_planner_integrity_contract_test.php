...
$widget=$read('views/partials/work-planner-widget.php');foreach(['planner_tomorrow','planner_urgent','انتقال به فردا'] as $token)if(!str_contains($widget,$token))$fail('Dashboard task action is missing: '.$token);if(str_contains($widget,'name="due_date"'))$fail('Dashboard quick add must use the server today date.');
...
if(!str_contains($planner,'markTaskUrgent'))$fail('Planner urgent action is missing.');
if(str_contains($planner,"'priority_changed'"))$fail('Planner urgent action must use a backward-compatible log action.');
