...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'template') {
            ...
            try {
                $data = [$title,trim((string)($_POST['category'] ?? '')),$unitId,trim((string)($_POST['description'] ?? '')),isset($_POST['active'])?1:0,max(0,(int)($_POST['sort_order'] ?? 0))];
                if ($id) Database::execute('UPDATE hr_kpi_templates SET title=?,category=?,org_unit_id=?,description=?,active=?,sort_order=?,updated_at=NOW() WHERE id=?', [...$data,$id]);
                else {
                    Database::execute('INSERT INTO hr_kpi_templates(title,code,category,org_unit_id,description,active,sort_order,created_at,updated_at) VALUES (?,?,?,?,?,?,?,NOW(),NOW())', [$title,$code,...array_slice($data,1)]);
