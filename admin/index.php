...
if($_SERVER['REQUEST_METHOD']==='POST'&&($_POST['action']??'')==='planner_quick_add'){try{if(!Auth::verifyCsrf($_POST['csrf_token']??''))throw new InvalidArgumentException('اعتبار فرم منقضی شده است.');$due=JalaliDate::toGregorian((string)($_POST['due_date']??''))?:date('Y-m-d');WorkPlannerService::createPersonalTask($dashboardUserId,$_POST,$due);flash('وظیفه امروز افزوده شد.');}catch(InvalidArgumentException $e){flash($e->getMessage(),'danger');}catch(Throwable $e){error_log('Dashboard planner quick add: '.$e->getMessage());flash('افزودن وظیفه انجام نشد.','danger');}redirect('/admin/index.php');}
$pageTitle='داشبورد';
