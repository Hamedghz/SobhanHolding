...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $rawDate = trim((string)($_POST['letter_date'] ?? ''));
        $date = app_date_to_iso($rawDate);
        $body = LetterModule::sanitizeHtml($_POST['body_html'] ?? '');
        $bodyDelta = LetterModule::sanitizeDelta($_POST['body_delta_json'] ?? '');
        ...
        if ($date === null) throw new InvalidArgumentException('تاریخ نامه معتبر نیست.');
        ...
        $values = [$letterNumber,$date,$subject,$recipient,trim((string)($_POST['recipient_title']??'')),trim((string)($_POST['recipient_organization']??'')),trim((string)($_POST['sender_unit']??'')),(int)($_POST['template_id']??0)?:null,(int)($_POST['letterhead_id']??0)?:null,(int)($_POST['signature_id']??0)?:null,$body,$bodyDelta,$paper,$orientation,$importance,$confidentiality];
        if ($id) {
            Database::execute('UPDATE organizational_letters SET letter_number=?,letter_date=?,subject=?,recipient_name=?,recipient_title=?,recipient_organization=?,sender_unit=?,template_id=?,letterhead_id=?,signature_id=?,body_html=?,body_delta_json=?,paper_size=?,orientation=?,importance=?,confidentiality=?,final_html=NULL,updated_at=NOW() WHERE id=? AND status="draft"', [...$values,$id]);
            LetterModule::log($id, 'updated', 'draft', 'draft');
        } else {
            Database::execute('INSERT INTO organizational_letters(letter_number,letter_date,subject,recipient_name,recipient_title,recipient_organization,sender_unit,template_id,letterhead_id,signature_id,body_html,body_delta_json,paper_size,orientation,importance,confidentiality,status,created_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,"draft",?,NOW(),NOW())', [...$values,(int)Auth::user()['id']]);
...
$letterheads = Database::fetchAll('SELECT id,title,company_name,is_default FROM letter_letterheads WHERE is_active=1' . ($letter && $letter['letterhead_id'] ? ' OR id='.(int)$letter['letterhead_id'] : '') . ' ORDER BY is_default DESC,title');
...
if (!$letter && empty($form['letterhead_id'])) foreach ($letterheads as $head) if ((int)$head['is_default'] === 1) {$form['letterhead_id']=(int)$head['id'];break;}
$form['body_html'] = LetterModule::sanitizeHtml($form['body_html']);
$templatePayload = [];
foreach ($templates as $template) $templatePayload[(string)$template['id']] = ['subject'=>$template['default_subject'],'body'=>$template['default_body'],'delta'=>$template['default_delta_json']??null,'letterhead_id'=>$template['letterhead_id'],'signature_id'=>$template['signature_id'],'paper_size'=>$template['paper_size'],'orientation'=>$template['orientation']];
...
<input type="hidden" name="csrf_token" value="<?= e(Auth::csrfToken()) ?>"><input type="hidden" name="id" value="<?= e((string)$id) ?>"><textarea name="body_html" data-editor-input hidden><?= e($form['body_html']) ?></textarea><textarea name="body_delta_json" data-editor-delta hidden><?=e($form['body_delta_json']??'')?></textarea>
...
<section class="card letter-form-section"><header><div><span class="letter-step">۳</span><h2>متن نامه</h2></div><button class="btn btn-small" type="button" data-preview-toggle>پیش‌نمایش</button></header>
    <div class="letter-editor" data-rich-editor><div class="editor-toolbar" data-quill-toolbar role="toolbar" aria-label="ابزار ویرایش متن"><select class="ql-font"><option selected></option><option value="serif"></option><option value="monospace"></option></select><select class="ql-size"><option value="small"></option><option selected></option><option value="large"></option><option value="huge"></option></select><button class="ql-bold" type="button"></button><button class="ql-italic" type="button"></button><button class="ql-underline" type="button"></button><select class="ql-color"></select><select class="ql-background"></select><button class="ql-direction" value="rtl" type="button"></button><select class="ql-align"></select><button class="ql-list" value="ordered" type="button"></button><button class="ql-list" value="bullet" type="button"></button><button class="ql-indent" value="-1" type="button"></button><button class="ql-indent" value="+1" type="button"></button><button class="ql-link" type="button"></button><button class="ql-clean" type="button"></button><button class="ql-undo" type="button" title="واگرد"></button><button class="ql-redo" type="button" title="از نو"></button></div><div class="editor-surface" dir="rtl" data-editor-surface><?= $form['body_html'] ?></div></div>
