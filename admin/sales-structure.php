...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'save_line') {
            ...
            if ($supervisorId) {
                SalesStructureModule::syncSupervisorCompatibilityFields($supervisorId, $id, (int)$currentUser['id']);
            }
            ...
            $validGeoRows = Database::fetchAll("SELECT id FROM sales_geographies WHERE active=1 AND id IN ({$placeholders})", $geoIds);
            $geoIds = array_map('intval', array_column($validGeoRows, 'id'));
            ...
            SalesStructureModule::syncUserCompatibilityFields($visitorId, $lineId, (int)$currentUser['id']);
...
$lines = SalesStructureModule::lines();
$brands = SalesStructureModule::lineBrands();
$geographies = SalesStructureModule::geographies();
$cities = array_values(array_filter($geographies, static fn($g) => ($g['type'] ?? '') === 'city'));
$territories = SalesStructureModule::visitorTerritories();
...
$diagnostics = SalesStructureModule::diagnostics();
...
<div class="stats">
    <div class="stat-card"><span>لاین بدون مدیر</span><strong><?=e((string)count($diagnostics['lines_without_manager']))?></strong></div>
    <div class="stat-card"><span>لاین بدون سرپرست</span><strong><?=e((string)count($diagnostics['lines_without_supervisor']))?></strong></div>
    <div class="stat-card"><span>ویزیتور بدون محدوده</span><strong><?=e((string)count($diagnostics['visitors_without_territory']))?></strong></div>
    <div class="stat-card"><span>فاقد منطقه زاهدان</span><strong><?=e((string)count($diagnostics['visitors_without_zahedan']))?></strong></div>
</div>

<?php if ($diagnostics['lines_without_manager'] || $diagnostics['lines_without_supervisor'] || $diagnostics['duplicate_supervisors'] || $diagnostics['visitors_without_territory'] || $diagnostics['visitors_without_zahedan'] || $diagnostics['multiple_primary']): ?>
<section class="card">
    <h2>هشدارهای تکمیل ساختار</h2>
    <p class="muted">این موارد ذخیره اطلاعات قدیمی را مسدود نمی‌کنند؛ از فرم‌های همین صفحه برای تکمیل آن‌ها استفاده کنید.</p>
    <div class="table-responsive"><table class="table"><thead><tr><th>نوع هشدار</th><th>موارد</th></tr></thead><tbody>
        <?php if ($diagnostics['lines_without_manager']): ?><tr><td>لاین بدون مدیر</td><td><?=e(implode('، ', array_column($diagnostics['lines_without_manager'], 'title')))?></td></tr><?php endif; ?>
        <?php if ($diagnostics['lines_without_supervisor']): ?><tr><td>لاین بدون سرپرست</td><td><?=e(implode('، ', array_column($diagnostics['lines_without_supervisor'], 'title')))?></td></tr><?php endif; ?>
        <?php if ($diagnostics['duplicate_supervisors']): ?><tr><td>سرپرست در چند لاین فعال</td><td><?php foreach($diagnostics['duplicate_supervisors'] as $row): ?><span class="badge"><?=e($row['line_codes'])?></span> <?php endforeach;?></td></tr><?php endif; ?>
        <?php if ($diagnostics['visitors_without_territory']): ?><tr><td>ویزیتور بدون محدوده</td><td><?=e(implode('، ', array_column($diagnostics['visitors_without_territory'], 'name')))?></td></tr><?php endif; ?>
        <?php if ($diagnostics['visitors_without_zahedan']): ?><tr><td>ویزیتور بدون منطقه فعال زاهدان</td><td><?=e(implode('، ', array_column($diagnostics['visitors_without_zahedan'], 'name')))?></td></tr><?php endif; ?>
        <?php if ($diagnostics['multiple_primary']): ?><tr><td>بیش از یک محدوده اصلی در یک لاین</td><td><?php foreach($diagnostics['multiple_primary'] as $row): ?><span class="badge"><?=e($row['visitor_name'].' / '.$row['line_code'])?></span> <?php endforeach;?></td></tr><?php endif; ?>
    </tbody></table></div>
</section>
<?php endif; ?>
