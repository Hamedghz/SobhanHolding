...
try{$job=AiUpdateService::createAndRun('test_connection',(int)Auth::user()['id']);echo json_encode(['ok'=>$job['status']==='completed','job'=>$job],JSON_UNESCAPED_UNICODE);}catch(Throwable $e){error_log('AI test action: '.$e->getMessage());echo json_encode(['ok'=>false,'message'=>'تست اتصال ناموفق بود.'],JSON_UNESCAPED_UNICODE);}
