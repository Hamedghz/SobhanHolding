...
$adminExtraStylesheets=['/assets/css/sales-report-forward.css'];
$canForwardReport=SalesReportShareService::canForward();
$forwardOptions=$canForwardReport?MessengerForwardService::recipientOptions():['users'=>[],'groups'=>[],'lines'=>[]];
...
<?php if($canForwardReport):?><div class="forward-summary-bar"><button class="btn" type="button" data-forward-report="summary_cards" data-forward-title="خلاصه فروش <?=e($report['report_title'])?>">ارسال خلاصه به پیام‌رسان</button><a class="btn btn-light" href="/messenger/">مشاهده پیام‌رسان</a></div><?php endif?>
...
<?php endforeach?>
<?php if((int)($aiSettings['ai_enabled']??0)&&Auth::can('manager_dashboard.ai_run')):$buttonLabels=['calculate_achievement'=>'محاسبه و کنترل تحقق','calculate_commission'=>'بررسی پورسانت‌ها','calculate_penalty'=>'بررسی ضرایب کاهنده','calculate_customer_coverage'=>'تحلیل پوشش مشتری','calculate_brand_target'=>'تحلیل تحقق برند','detect_anomalies'=>'شناسایی مغایرت‌ها','generate_management_recommendations'=>'پیشنهاد اقدام مدیریتی'];?><section class="card manager-ai" id="widget-ai_insights"><h2>بینش هوشمند فروش</h2><p>تحلیل فقط خواندنی است و هیچ داده‌ای را تغییر نمی‌دهد.</p><?php if($canForwardReport):?><button class="btn btn-forward" type="button" data-forward-report="ai_analysis" data-forward-title="تحلیل هوش مصنوعی فروش">ارسال تحلیل به پیام‌رسان</button><?php endif?><?php if((int)($aiSettings['ai_show_buttons']??1)):?><form method="post"><input type="hidden" name="csrf_token" value="<?=e(Auth::csrfToken())?>"><input type="hidden" name="action" value="ai"><div class="actions"><button class="btn btn-light" name="skill_key" value="basic_summary">تحلیل کلی گزارش</button><?php if((int)($aiSettings['ai_skills_enabled']??0)): foreach(ManagerDashboard::enabledSkills() as $skill):?><button class="btn btn-light" name="skill_key" value="<?=e($skill['skill_key'])?>"><?=e($buttonLabels[$skill['skill_key']]??$skill['skill_title_fa'])?></button><?php endforeach; endif?></div></form><?php endif?><?php if(!empty($aiAnswer)):?><div class="ai-answer"><?=md_render_ai_answer($aiAnswer)?><?=render_ai_knowledge_sources($aiKnowledgeSources??[])?></div><?php endif?></section><?php endif?>
<?php if($canForwardReport):?>
<dialog class="forward-dialog" id="salesForwardDialog"><form id="salesForwardForm">
<header><div><span>Snapshot امن و فقط‌خواندنی</span><h2>ارسال گزارش به پیام‌رسان</h2></div><button type="button" data-forward-close aria-label="بستن">×</button></header>
<input type="hidden" name="csrf_token" value="<?=e(Auth::csrfToken())?>"><input type="hidden" name="action" value="send"><input type="hidden" name="report_id" value="<?=$reportId?>"><input type="hidden" name="report_type" value="summary_cards"><input type="hidden" name="filters_json" value="<?=e(json_encode(['search'=>$_GET['search']??'','line_code'=>$_GET['line_code']??'','visitor'=>$_GET['visitor']??'','supervisor'=>$_GET['supervisor']??''],JSON_UNESCAPED_UNICODE))?>">
<div class="forward-grid"><section><label>عنوان پیام<input name="title" maxlength="190" required></label><label>نوع گیرنده<select name="recipient_type" data-recipient-type><option value="single_user">یک کاربر</option><option value="multiple_users">چند کاربر</option><option value="group">گروه پیام‌رسان</option><option value="sales_line">لاین فروش</option><option value="supervisors">سرپرستان</option><option value="managers">مدیران</option></select></label>
<div data-recipient-panel="single_user"><label>کاربر<select name="recipient_id"><option value="">انتخاب کنید</option><?php foreach($forwardOptions['users'] as $option):?><option value="<?=$option['id']?>"><?=e($option['name'].($option['department']?' · '.$option['department']:''))?></option><?php endforeach?></select></label></div>
<div data-recipient-panel="multiple_users" hidden><label>کاربران<select name="recipient_ids[]" multiple size="7"><?php foreach($forwardOptions['users'] as $option):?><option value="<?=$option['id']?>"><?=e($option['name'].($option['department']?' · '.$option['department']:''))?></option><?php endforeach?></select></label></div>
<div data-recipient-panel="group" hidden><label>گروه<select name="group_id"><option value="">انتخاب کنید</option><?php foreach($forwardOptions['groups'] as $option):?><option value="<?=$option['id']?>"><?=e($option['title'])?></option><?php endforeach?></select></label><?php if(!$forwardOptions['groups']):?><small>هنوز گروه فعالی تعریف نشده است.</small><?php endif?></div>
<div data-recipient-panel="sales_line" hidden><label>لاین فروش<select name="sales_line"><option value="">انتخاب کنید</option><?php foreach($forwardOptions['lines'] as $option):?><option value="<?=e($option)?>"><?=e($option)?></option><?php endforeach?></select></label></div>
<label>نوع خروجی<select data-output-type><option value="source">جدول کامل / خروجی انتخاب‌شده</option><option value="summary_cards">خلاصه</option><option value="chart">نمودار</option><option value="attachment">فایل پیوست</option><option value="full_report">گزارش کامل</option><option value="ai_analysis">تحلیل هوش مصنوعی</option></select></label><label>توضیح اختیاری<textarea name="description" rows="4" maxlength="4000"></textarea></label><label class="forward-check"><input type="checkbox" name="include_attachment" value="1"><span>فایل CSV جدول نیز پیوست شود</span></label></section>
<aside><h3>پیش‌نمایش Snapshot</h3><div class="forward-preview" data-forward-preview>برای ساخت پیش‌نمایش، نوع گزارش را انتخاب کنید.</div><button class="btn btn-light" type="button" data-forward-preview-button>بروزرسانی پیش‌نمایش</button></aside></div>
<p class="forward-status" data-forward-status role="status"></p><footer><button class="btn" type="submit">ارسال</button><button class="btn btn-light" type="button" data-forward-close>انصراف</button></footer>
</form></dialog>
<script src="/assets/js/sales-report-forward.js"></script>
<?php endif?>
