...
<section class="card">
    ...
    <tr><td><strong><?=e($label)?></strong></td><td><input type="checkbox" name="modules[<?=e($key)?>][enabled]" value="1" <?=($row?(int)$row['enabled']:1)?'checked':''?>></td><td><input type="checkbox" name="modules[<?=e($key)?>][desktop_enabled]" value="1" <?=($row?(int)$row['desktop_enabled']:1)?'checked':''?>></td><td><input type="checkbox" name="modules[<?=e($key)?>][show_body]" value="1" <?=($row?(int)$row['show_body']:$defaultShow)?'checked':''?>></td><td><select name="modules[<?=e($key)?>][sound]"><?php foreach(['default'=>'پیش‌فرض','important'=>'مهم','message'=>'پیام','silent'=>'بی‌صدا'] as $v=>$t):?><option value="<?=$v?>" <?=($row['sound']??'default')===$v?'selected':''?>><?=$t?></option><?php endforeach?></select></td><td><select name="modules[<?=e($key)?>][priority]"><?php foreach(['low'=>'کم','normal'=>'عادی','important'=>'مهم'] as $v=>$t):?><option value="<?=$v?>" <?=($row['priority']??'normal')===$v?'selected':''?>><?=$t?></option><?php endforeach?></select></td><td><?php if($key==='messenger'):?><label><input type="checkbox" name="modules[<?=$key?>][allow_quick_reply]" value="1" <?=!empty($row['allow_quick_reply'])?'checked':''?>> پاسخ سریع</label><?php elseif($key==='approval'):?><label><input type="checkbox" name="modules[<?=$key?>][direct_action_enabled]" value="1" <?=!empty($row['direct_action_enabled'])?'checked':''?>> تأیید/رد مستقیم</label><?php else:?>—<?php endif?></td></tr>
